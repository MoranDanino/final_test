...
def home():
   ...
   # Fetch VPCs
   vpcs = ec2_client.describe_vpcs()  #get the list of vpcs in our AWS account 
   vpc_data = [{"VPC ID": vpc["VpcId"], "CIDR": vpc["CidrBlock"]} for vpc in vpcs["Vpcs"]]
  
   # Fetch Load Balancers
   lbs = elb_client.describe_load_balancers() #get the list of lbs in our AWS account 
   lb_data = [{"LB Name": lb["LoadBalancerName"], "DNS Name": lb["DNSName"]} for lb in lbs["LoadBalancers"]]
  
   # Fetch AMIs (only owned by the account)
   amis = ec2_client.describe_images(Owners=["self"]) #get the list of the amis in our AWS account 
